...
@ReqFields({
	...
	@ReqField(name = "passwordCfm", option = false),
...
public class Register extends BaseAction {
	...
	public void doTrsPre(Model model) throws SeException {
		...
		//检查两遍密码是否输入一致
		if(!model.getData("password").equals(model.getData("passwordCfm"))) {
			throw new SeException("确认密码与密码不一致");
		}
	}
	...
	public void doTrs(Model model) throws SeException {
		...
		newUser.setPassword((String)model.getData("password"));
